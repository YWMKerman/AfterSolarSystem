...
// 这个配置文件来源于RSS,抱歉，我只能这么做，以为我一个人永远不可能做得了这些文件，我只能使用RSS官方的，但我会著名是你们的
...
namespace AfterSolarSystem
{
    ...
    public class ASSWatchDog : MonoBehaviour
    {
        ...
        public void Start()
        {
            foreach (ConfigNode node in GameDatabase.Instance.GetConfigNodes("AFTERSOLARSYSTEM"))
                ASSSettings = node;
        ...
        public void Update()
        {
            ...
            {
                clipPlaneSettings = ASSSettings.GetNode("ClipPlanes")?.GetNode("DX11");
            }
            else
            {
                clipPlaneSettings = ASSSettings.GetNode("ClipPlanes")?.GetNode("Other");
            }
            ...
            foreach (Camera cam in cameras)
            {
                ...
                if (nearClip > 0)
                {
                    ...
                    Debug.Log($"[AfterSolarSystem] Watchdog: Setting camera {cam.name} near clip to {nearClip} so camera now has {cam.nearClipPlane}");
                }

                if (farClip > 0)
                {
                    cam.farClipPlane = farClip;
                    Debug.Log($"[AfterSolarSystem] Watchdog: Setting camera {cam.name} far clip to {farClip} so camera now has {cam.farClipPlane}");
                }
            }
        }
        ...
        private void OnVesselSituationChanged(GameEvents.HostedFromToAction<Vessel, Vessel.Situations> data)
        {
            ...
            {
                isSuborbital = false;
                Debug.Log("[AfterSolarSystem] Calling StartUpSphere() to prevent missing PQ tiles");
