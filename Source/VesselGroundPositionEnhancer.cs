        ...
        private void OnVesselGoOffRails(Vessel v)
        {
            if (v.Landed && !v.Splashed)
            {
                if (v.situation == Situations.PRELAUNCH)
                {
                    ...
                    Debug.Log($"[ASS-VGPE] Vessel going off rails in PRELAUNCH, skipping {framesToSkip} frames on CollisionEnhancer.");
                }
                else
                {
                    ...
                    Debug.Log($"[ASS-VGPE] CheckGroundCollision() in {sw.ElapsedMilliseconds}ms");
                    sw.Stop();
                }
            }
            else
            {
                Debug.Log("[ASS-VGPE] not landed");
            }
        }
    }
}
